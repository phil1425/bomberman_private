    ...
    for game_state, action in zip(game_state_list, action_list):
        for mirror in [False, True]:
            ...
            if mirror:
                ...
                game_state_mirrored['bombs'] = [((b[0][0], -b[0][1]), b[1]) for b in game_state['bombs']]
                game_state_mirrored['explosion_map'] = np.flip(game_state['explosion_map'], axis=-1)
                game_state_mirrored['coins'] = [(b[0], -b[1]) for b in game_state['coins']]
                b = game_state_mirrored['self']
                game_state_mirrored['self'] = (b[0], b[1], b[2], (b[3][0], -b[3][1]))
                game_state_mirrored['others'] = [(b[0], b[1], b[2], (b[3][0], -b[3][1])) for b in game_state['others']]
            ...
            for rot in [0, 1, 2, 3]:
                ...
                for _ in range(rot):
                    ...
                    game_state_rotated['bombs'] = [((b[0][1], -b[0][0]), b[1]) for b in game_state_rotated['bombs']]
                    game_state_rotated['explosion_map'] = np.rot90(game_state_rotated['explosion_map'], 3)
                    game_state_rotated['coins'] = [(b[1], -b[0]) for b in game_state_rotated['coins']]
                    b = game_state_rotated['self']
                    game_state_rotated['self'] = (b[0], b[1], b[2], (b[3][1], -b[3][0]))
                    game_state_rotated['others'] = [(b[0], b[1], b[2], (b[3][1], -b[3][0])) for b in game_state_rotated['others']]
...
if __name__ == '__main__':
    ...
    states = pickle.load(open('game_state.p', 'rb'))
    actions = pickle.load(open('action.p', 'rb'))
    
    aug_states, aug_actions = augment_game_states(states, actions)
    for i in range(0):
        for l in range(8):
            print(l, ['UP', 'DOWN', 'LEFT', 'RIGHT', 'BOMB', 'WAIT'][np.argmax(aug_actions[i*8+l])])
        for j in range(12):
            plt.figure()
            for k in range(8):
                plt.subplot(int(420+k+1))
                data = to_features(aug_states[i*8+k])
                plt.imshow(data[:,:,j])
        plt.show()
