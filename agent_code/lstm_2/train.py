...
def game_events_occurred(self, old_game_state, action, new_game_state, events):
    ...
    if old_game_state != None:
        ...
        if e.INVALID_ACTION in events or e.WAITED in events:
            ...
            self.aux_reward = add_to_list(self.aux_reward, 0, 1)
...
def end_of_round(self, last_game_state, last_action, events):
    ...
    reward = aux_reward+(self.num_boxes_destroyed+last_game_state['self'][1]*2)/last_game_state['step']
    ...
    pickle.dump(self.action, open('action.p', 'wb'))
