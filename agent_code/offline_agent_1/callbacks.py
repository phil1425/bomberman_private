...
class selectiveModel():
    ...
    def fit(self, X, y):
        ...
        self.model.fit(self.select_features(X).reshape(X.shape[0], 7*7*12), y)

    def predict(self, X):
        return self.model.predict(self.select_features(X).reshape(X.shape[0], 7*7*12))
...
def setup(self):
    self.model = load_model('model', custom_objects={'sparse_crossentropy_masked':sparse_crossentropy_masked})
