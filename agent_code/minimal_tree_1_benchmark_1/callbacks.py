...
def act(self, game_state: dict):
    if self.train:
        if game_state['round'] == 1:
            ...
            epsilon = 0.01+0.9*np.exp(-game_state['round']/200)
    else:
        epsilon = 0.01

    if np.random.random() > epsilon:
        state, action, rot, mir = rotate_game_states([game_state], [np.array([0,0,0,0,0,0])])
        features =  self.feature_selector.eval_coin_features(state[0])
    ...
    if not self.train:
        if game_state['round']%200 == 0 and game_state['step'] == 1:
            self.actor.load('actor.p')

    if self.prev_step > game_state['step']:
        self.score.append(self.prev_score)
    
    self.logger.info(np.mean(self.score))
    
    self.prev_step = game_state['step']
